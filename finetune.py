...
cnn_dailymail_train = np.load('finetune/cnn_dailymail_train.npy', allow_pickle=True)
cnn_dailymail_validation = np.load('finetune/cnn_dailymail_validation.npy', allow_pickle=True)
cnn_dailymail_test = np.load('finetune/cnn_dailymail_test.npy', allow_pickle=True)

SQuAD_train = np.load('finetune/SQuAD_train.npy', allow_pickle=True)
SQuAD_validation = np.load('finetune/SQuAD_validation.npy', allow_pickle=True)

vocab_size = len(stoi_comm)
print('Vocab size:', vocab_size)

print('cnn_dailymail_train:', len(cnn_dailymail_train))
print('cnn_dailymail_validation:', len(cnn_dailymail_validation))
print('cnn_dailymail_test:', len(cnn_dailymail_test))
print('SQuAD_train:', len(SQuAD_train))
print('SQuAD_validation:', len(SQuAD_validation))

# Dataloader for finetune training
class DataLoaderForFinetune(torch.utils.data.DataLoader): 
    def __init__(self, dataset1, dataset2, seq_len, batch_size, num_workers, shuffle=True, drop_last=False):
        self.seq_len     = seq_len
        self.batch_size  = batch_size
        self.shuffle     = shuffle
        self.drop_last   = drop_last
        self.num_workers = num_workers
        self.dataset     = np.append(np.concatenate(dataset1), np.concatenate(dataset2), axis=0)
...
# Dataloader for finetune validation
class DataLoaderForFinetuneVal(torch.utils.data.DataLoader):
    def __init__(self, dataset1, dataset2, num_workers, shuffle=True):
    ...
    def __iter__(self):
        if self.shuffle:
            np.random.shuffle(self.dataset1)
            np.random.shuffle(self.dataset2)

        for i in range(len(self.dataset1)):
            yield torch.tensor(self.dataset1[i, 0]), torch.LongTensor(self.dataset1[i, 1])

        for i in range(len(self.dataset2)):
            yield torch.tensor(np.concatenate(self.dataset2[i,:-1])), torch.LongTensor(self.dataset2[i,-1])

# Dataloader
train_loader = DataLoaderForFinetune(
    dataset1    = cnn_dailymail_train,
    dataset2    = SQuAD_train,
    seq_len     = 500,
    batch_size  = 20,
    shuffle     = True,
    num_workers = 16
...
val_loader = DataLoaderForFinetuneVal(
    dataset1    = cnn_dailymail_validation[:5],
    dataset2    = SQuAD_validation[:5],
    shuffle     = False,
    num_workers = 16
)
...
for i, (x, y) in enumerate(val_loader):
    x, y = x.to(device), y.to(device)
    with torch.no_grad():
        prediction = model.generate(x.view(1, -1), len(y))
        target = decode(np.array(y.to('cpu').unsqueeze(0).T), itos_comm)
        output = decode(np.array(prediction.to('cpu').unsqueeze(0).T), itos_comm)
    break
